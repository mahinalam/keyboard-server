...
const updateProductQuantityIntoDB = async (
  ...
  const existingProductQuantity = product?.availableQuantity
  const newProductQuantity = payload.quantity
  if (existingProductQuantity < newProductQuantity) {
    throw new AppError(httpStatus.BAD_REQUEST, 'Insufficient Product Quantity!')
  }
  const updatedProductQuantity = Number(
    existingProductQuantity - newProductQuantity,
  )
  const result = await Product.findByIdAndUpdate(
    id,
    { availableQuantity: updatedProductQuantity },
    {
      new: true,
    },
  )
